...
save.addEventListener("click", () => {
	localStorage.setItem("pontos", pontos); // Converte o estado dos nos em string 
	localStorage.setItem("posicoes", JSON.stringify(data.nodes.map(({ id, group }) => ({ id, group }))));
});
load.addEventListener("click", () => {
	pontos = localStorage.getItem("pontos");
	JSON.parse(localStorage.getItem("posicoes")).forEach(nodePosition => data.nodes.update(nodePosition));
	updatepontosDisplay();
});
